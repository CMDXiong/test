...
wxPropertyValue::wxPropertyValue(const wxString& val)
{
  ...
  m_value.string = strdup( val.c_str() );
  m_clientData = NULL;
  m_next = NULL;
  m_last = NULL;
}
...
wxPropertyValue::wxPropertyValue(wxStringList *the_list)
{
  ...
  wxStringListNode *node = the_list->GetFirst();
  while (node)
  {
...
void wxPropertyValue::operator=(const wxString& val1)
{
  ...
  {
    if (val)
      m_value.string = strdup( val1.c_str() );
    else
      m_value.string = NULL;
  }
