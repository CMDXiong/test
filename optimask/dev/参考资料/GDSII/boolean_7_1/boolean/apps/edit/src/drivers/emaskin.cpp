 ...
 * Comment        The Flash files I have seen suggest that the first and last
                  ...
                  then some of the driver must be rewritten. Especially the
						Read_Command_From_EMASK_File function. Also bear in mind
...
#define  EMASK_U_UNIT_VALUE    0.1         // 1E-1
#define  EMASK_P_UNIT_VALUE    0.0000001   // 1E-7
...
//CONSTRUCTOR-------------------------------------------------------------------
EMASK_driver_in::EMASK_driver_in(const wxString& filename, bool add, int layernr)
{

	EMASK_filename=filename;
	SetDavidMannAdd(add);
	...
	EMASK_file.open(EMASK_filename, ios::in);
	if (EMASK_file.bad())
	{
		char errbuf[100];
		sprintf(errbuf, "Cannot open file %s!",EMASK_filename.c_str() );
...
//DESTRUCTOR--------------------------------------------------------------------
EMASK_driver_in::~EMASK_driver_in()
{
	EMASK_file.close();
}

//Read GdsII (Main routine)-----------------------------------------------------
void EMASK_driver_in::Read(GdsII* _GdsII)
{
...
void EMASK_driver_in::Read_GDSII(GdsII* _GdsII)
{
	if (!GetDavidMannAdd())
	{
...
// READ BGNLIB------------------------------------------------------------------
void EMASK_driver_in::Read_Bgnlib(GdsII* _GdsII)
{
...
// READ LIBRARY-----------------------------------------------------------------
void EMASK_driver_in::Read_Library(GdsII* _GdsII)
{
// set librarysetting
	Library* _library = _GdsII->GetLibrary();
	_library->SetLibraryName(LIBRARY_NAME);
	_GdsII->SetUserUnits(EMASK_U_UNIT_VALUE);
	_GdsII->SetPhysicalUnits(EMASK_P_UNIT_VALUE);
}

// READ STRUCTURE
void EMASK_driver_in::Read_Structure(GdsII* _structurelist)
{
	...
	if (GetDavidMannAdd())
	{
		...
		while (Read_Command_From_EMASK_File())
...
// READ BGNSTR------------------------------------------------------------------
void EMASK_driver_in::Read_Bgnstr(Structure* _structure)
{
...
void EMASK_driver_in::Read_StructureName(Structure* _structure)
{
...
bool EMASK_driver_in::Read_Command_From_EMASK_File ()
{
	int teller =0;
	char ch = '\0';

   bool commentflag=false;
	while(  (EMASK_file.get(ch)) &&			// GET A CHARACTER
   		  ( !( ((ch == 'S') || (ch =='Z')) &&
          	 	 (!commentflag) //ignore Z and S inside Comment
              )
           ) &&	// end of command or endof file (and !comment)
		  	  (EMASK_file.eof()==0 )	// test voor eof
         )

// read the file and fill the buffer with the command found.
// file is read until a semi-collumn is found.
	{
      //ignore characters that are no commands and also comments
         switch (ch)
         {
            case '\n' :
            case '.'  :
      ...
      if (teller>=EMASK_BUFF_SIZE-1)// to test if the buffer exceeds the limits
      {
         char errbuf[100];
         sprintf(errbuf, "File %s!\nIs NOT a valid Emask File",EMASK_filename.c_str());
         throw GDS_Error(errbuf, "flashprs: File Error", 9, 0);
      }
	}
	switch (ch)
	{
		case 'S' : {
						command_buffer[teller] = 'S';
						return true;
					  }
		case 'Z' : {
						command_buffer[teller] = 'Z';
						return false;
					  }
		default :
		{
			char errbuf[100];
			sprintf(errbuf, "'%c! in file indicates that File %s! is not a Emask file ", ch, EMASK_filename.c_str());
			throw GDS_Error(errbuf, "flashprs: File Error", 9, 0);
		}
	}
}
...
// STRIP BLANKS-----------------------------------------------------------------
void EMASK_driver_in::Strip_Blanks()
{
	int teller =0;
	while (command_buffer[teller] != 'S')
	{
...
void EMASK_driver_in::Strip_Item(int a)
{
	while (command_buffer[a+1] != 'S')
	{
...
void EMASK_driver_in::Read_Items(int &teller) // misschien met een bool??
{
// check for each command if it is there
	if (is_Keyword(command_buffer[teller]))
	{
		...
		else if ((command_buffer[teller]== 'V')||(command_buffer[teller]== 'v'))
		{
		...
		else if ((command_buffer[teller]== 'U')||(command_buffer[teller]== 'u'))
		{
			// waarde van w inlezen tot het volgende commando
			teller++; //keyword zelf weghalen.
			valuestring = ReadValue(teller);
		...
		else if (command_buffer[teller]== 'S')
		{
			return; // Good way out.
		}
	}
...
bool EMASK_driver_in::is_Keyword(char ch)
{
	switch (ch)
	{
		...
		case 'w' : case 'W' :
		case 'u' : case 'U' :
		case 'S' : // and the terminator, not really a command but necessary
		{
			return true;
		}
		default :
...
char* EMASK_driver_in::ReadValue(int &teller)
{
	int vsteller =0;
...
void EMASK_driver_in::Read_Element(Structure* _structure)
{
...
void EMASK_driver_in::Read_Box(Structure* _structure)
{
// <box>	::=	BOX [ELFLAGS] [PLEX] LAYER BOXTYPE [WIDTH] XY
	...
	double w = Getlast_Width()*1e-7/EMASK_P_UNIT_VALUE;
	double h = Getlast_Height()*1e-7/EMASK_P_UNIT_VALUE;
	double x = Getlast_Xpos()*1e-7/EMASK_P_UNIT_VALUE;
	double y = Getlast_Ypos()*1e-7/EMASK_P_UNIT_VALUE;
   _box->Set(x,y,a,w,h);
...
// READ ELFLAGS-----------------------------------------------------------------
void EMASK_driver_in::ReadElflags(Element* _element)
{
}

// READ LAYER-------------------------------------------------------------------
void EMASK_driver_in::ReadLayer(Shape* _shape)
{
...
// READ BOXTYPE-----------------------------------------------------------------
void EMASK_driver_in::ReadBoxType(Box* _box)
{
	_box->SetBoxtype(0);
}

// READ WIDTH-------------------------------------------------------------------
void EMASK_driver_in::ReadWidth(Shape* _shape)
{
	_shape->SetWidth(0);
}

//DEG2RAD-----------------------------------------------------------------------
double  EMASK_driver_in::Deg2Rad(int value)
{
...
double EMASK_driver_in::Inch2Meter(int value)
{
	return value/inch;
//end inch2meter
}
...
// GetAddAtLayer
int EMASK_driver_in::GetAddAtLayer()
{
	return AddAtLayer;
}

//	GET DAVID MANN ADD-----------------------------------------------------------
bool	EMASK_driver_in::GetDavidMannAdd()
{
	return DavidMannAdd;
}

//GET LAST X-POSITION-----------------------------------------------------------
int EMASK_driver_in::Getlast_Xpos()
{
	return last_Xpos;
}

//GET LAST Y-POSITION-----------------------------------------------------------
int EMASK_driver_in::Getlast_Ypos()
{
	return last_Ypos;
}

//GET LAST HEIGHT---------------------------------------------------------------
int EMASK_driver_in::Getlast_Height()
{
	return last_Height;
}

//GET LAST WIDTH----------------------------------------------------------------
int EMASK_driver_in::Getlast_Width()
{
	return last_Width;
}

//GET LAST ANGLE----------------------------------------------------------------
int  EMASK_driver_in::Getlast_Angle()
{
	return last_Angle;
}
...
void	EMASK_driver_in::SetAddAtLayer(int value)
{
	AddAtLayer = value;
}

//	SETDAVIDMANNADD--------------------------------------------------------------
void	EMASK_driver_in::SetDavidMannAdd(bool value)
{
	DavidMannAdd = value;
}

//SET LAST X-POSITION-----------------------------------------------------------
void EMASK_driver_in::Setlast_Xpos(int value)
{
 last_Xpos = value;
}

//SET LAST Y-POSITION-----------------------------------------------------------
void EMASK_driver_in::Setlast_Ypos(int value)
{
 last_Ypos = value;
}

//SET LAST HEIGHT---------------------------------------------------------------
void EMASK_driver_in::Setlast_Height(int value)
{
 last_Height = value;
}

//SET LAST WIDTH----------------------------------------------------------------
void EMASK_driver_in::Setlast_Width(int value)
{
 last_Width = value;
}

//SET LAST ANGLE----------------------------------------------------------------
void EMASK_driver_in::Setlast_Angle(int value)
{
 last_Angle = value;
}
//------------------------------------------------------------------------------
